...
<div class="uk-margin uk-grid-small" uk-grid>
    <div class="uk-width-2-3@s">
        <form class="form-horizontal" method="POST" action="{{ route('administrator.picks.allAdd') }}">
            {{ csrf_field() }}
...
@if( count($articles) > 0 )
<form class="form-horizontal" method="POST" action="{{ route('administrator.picks.allSave') }}">
    {{ csrf_field() }}
    <div class="uk-margin uk-grid-small uk-child-width-1-3@m sortable" uk-grid uk-sortable="handle: .uk-sortable-handle">
        ...
        <div id="picked_{{$article->id}}">
            <input type="hidden" class="input_on_top" name="on_top[]" value="{{$article->id}}">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    $('.autocomplete').autocomplete({
        source: function( req, res ) {
            window.axios.get("{{route('api.articles.search')}}?not_on_top=true&title=" + encodeURIComponent(req.term))
            .then(response => {
